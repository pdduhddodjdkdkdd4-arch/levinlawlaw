...
(function($){jQuery(document).on('ready',function($){"use strict";$('ul#nav li a[href^="#"]').on('click',function(event){var id=$(this).attr("href");var offset=60;if(id!='#'){var target=$(id).offset().top-offset;$('html, body').animate({scrollTop:target},1500,"easeInOutExpo")}
event.preventDefault()});var Body_Class=$('body');var Menu_Has_Children=$('.menu-item-has-children');var window_width=$(window).width();if(window_width<992){Body_Class.addClass('menu__mobile__style')}else{Body_Class.removeClass('menu__mobile__style')}
...
jQuery('.stellarnav').stellarNav({theme:'light',breakpoint:991,menuLabel:'',sticky:!1,position:style,openingSpeed:250,closingDelay:250,showArrows:!0,phoneBtn:contact,phoneLabel:'Call Us',locationBtn:location,locationLabel:'Location',closeBtn:!1,closeLabel:'Close',mobileMode:!1,scrollbarFix:!1});var hamburger=$('.header-top-area svg');$('a.menu-toggle').on('click',function(){var menuclass=$('#main-nav').attr('class');if('stellarnav light mobile active'==menuclass||'stellarnav light left mobile active'==menuclass||'stellarnav light right mobile active'==menuclass){hamburger.addClass('active')}else if('stellarnav light mobile'==menuclass||'stellarnav light left mobile'==menuclass||'stellarnav light right mobile'==menuclass){hamburger.removeClass('active')}
return!1});var hamber_toggle_icon=$('.humberger__menu__toggle');$('a.menu-toggle').append(hamber_toggle_icon);$('a.close-menu').on('click',function(){hamburger.removeClass('active');return!1});$(window).on('resize',function(){var menuclass=$('#main-nav').attr('class');if('stellarnav light desktop'==menuclass||'stellarnav light left desktop'==menuclass||'stellarnav light right desktop'==menuclass){hamburger.removeClass('active')}});if(window_width>992){var currentItem=$('ul#nav > li.current-menu-item');if(currentItem.length>0){currentItem.append('<div class="wee"></div>');var $thisnav=currentItem.offset().left;$('ul#nav > li.menu-item').on({mouseenter:function(){var $left=$(this).offset().left-$thisnav;var $width=$(this).outerWidth();var $start=0;$('.wee').css({'left':$left,'width':$width})},mouseleave:function(){var $initwidth=currentItem.width();$('.wee').css({'left':'0','width':$initwidth})},})}}
var win=$(window);resizeHandler();win.on('resize',resizeHandler());var window_width=win.width();win.on('resize',function(){var window_width=win.width();if(window_width>992){$('ul#nav li a[href^="#"]:not(.dd-toggle,a[href="#"])').on('click',function(event){$('ul#nav').css('display','inherit')})}});function resizeHandler(){if(window_width<=992){$('ul#nav li a[href^="#"]:not(.dd-toggle,a[href="#"])').on('click',function(event){$('#main-nav').removeClass('active');$('ul#nav').css('display','none');hamburger.removeClass('active')})}}
$('ul#offanvas__menu li a[href^="#"]').on('click',function(event){$('#offcanvas__mobile__menu__main__wrapper').removeClass('active').addClass('inactive')});var has_link=$('ul#nav li a[href^="#"]');has_link.on('click',function(event){console.log($(this).attr('href'))});$('#offcanvas__menu__open__trigger').on('click',function(){$('#offcanvas__mobile__menu__main__wrapper').removeClass('inactive').addClass('active')});$('#offcanvas__menu__close__trigger').on('click',function(){$('#offcanvas__mobile__menu__main__wrapper').removeClass('active').addClass('inactive')});var offCanvasNav=$('.offcanvas__navigation'),SubMenu=offCanvasNav.find('.sub-menu');SubMenu.parent().prepend('<span class="menu-expand"><i></i></span>');SubMenu.slideUp();offCanvasNav.on('click','li a, li .menu-expand',function(e){var $this=$(this);if(($this.parent().attr('class').match(/\b(menu-item-has-children|has-children|has-sub-menu)\b/))&&($this.attr('href')==='#'||$this.hasClass('menu-expand'))){e.preventDefault();if($this.siblings('ul:visible').length){$this.parent('li').removeClass('active');$this.siblings('ul').slideUp()}else{$this.parent('li').addClass('active');$this.closest('li').siblings('li').removeClass('active').find('li').removeClass('active');$this.closest('li').siblings('li').find('ul:visible').slideUp();$this.siblings('ul').slideDown()}}});var expanding_toggle=$('.expanding__nav__toggle'),close_expanding_toggle=$('.colse__expanding__nav__toggle'),body_class=$('body'),add_expand_class='menu__expand';expanding_toggle.on('click',function(e){body_class.addClass(add_expand_class);e.preventDefault()});close_expanding_toggle.on('click',function(e){body_class.removeClass(add_expand_class);e.preventDefault()});var language_changer=$('.language-button');language_changer.on('click',function(){$('.language__list').slideToggle()});var searchForm=$(document).find('#search-form-one').attr('class');if('search-form-one'==searchForm){$('.search-button').on('click',function(){$('body').addClass('mode-search');$('.search-form-one input[type="text"]').focus();$(this).addClass('close-search');return!1});$('.search-mode-close').on('click',function(){$('body').removeClass('mode-search');$('.search-button').removeClass('close-search');return!1})}
...
if($totalHeight+$(window).height()===$(document).height()){$scrollToTop.css("bottom","90px")}else{$scrollToTop.css("bottom","20px")}});var $container=$('.blog__grid,.download__grid');$container.imagesLoaded(function(){$container.masonry()});$(".post-media,.post-content").fitVids();var select_Ids=$('.edd_price_options select,.single-widgets select:not(.widget_product_categories select,.widget_layered_nav select),.download__filter__edd__sorting select,.download__orderby__shoring__filter select,select:not(.widget_product_categories select,.widget_layered_nav select)');$('select').addClass('wide');select_Ids.niceSelect();$('.nice-select').after('<div class="clearfix"></div>');Placeholdem(document.querySelectorAll('[placeholder]'));var postCarousel=$('.posts-gallery');if(postCarousel.length>0){postCarousel.owlCarousel({merge:!0,smartSpeed:1000,loop:!0,nav:!0,center:!1,dots:!1,navText:['<i class="fa fa-angle-left"></i>','<i class="fa fa-angle-right"></i>'],autoplay:!0,autoplayTimeout:3000,margin:0,responsiveClass:!0,responsive:{0:{items:1},600:{items:1},1000:{items:1},1200:{items:1}}})}
var utech_lagy_switch='0';if(typeof utech_lagy_load!=='undefined'){if(typeof utech_lagy_load.utech_lagy_switch!='undefined'){utech_lagy_switch=utech_lagy_load.utech_lagy_switch}}
if(utech_lagy_switch===1){var img_tag=$("img");var img_link='';img_tag.each(function(index){img_link=$(this).attr('src');$(this).attr('data-src',img_link)});img_tag.loadScroll(500)}}(jQuery))})(jQuery);(function($){jQuery(window).on('load',function(){"use strict";$(".preeloader").fadeOut(100);setTimeout(function(){$(".preeloader").hide()},100);new WOW().init({boxClass:'wow',offset:50,mobile:!1,live:!0});function concatHashToString(hash){var emptyStr='';$.each(hash,function(index){emptyStr+=' '+hash[index].name+'="'+hash[index].value+'"'});return emptyStr}
